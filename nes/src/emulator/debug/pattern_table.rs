...
impl PatternTableViewer {
    pub fn new(video: &sdl2::VideoSubsystem) -> Result<Self, Box<dyn std::error::Error>> {
        let window = video
            .window("Pattern table viewer", WINDOW_WIDTH, WINDOW_HEIGHT)
            .build()?;
    ...
    pub fn update(&mut self, nes: &super::super::Nes<'_>) {
        ...
        pt1.with_lock(None, |buf, pitch| {
            for tr in 0..16 {
                for tc in 0..16 {
                    ...
                    let bp_lo = &pattern_table[tile_addr..tile_addr + 8];
                    for y in 0..8 {
                        let byte_hi = bp_hi[y];
                        let byte_lo = bp_lo[y];
                        for x in 0..8 {
        ...
        self.canvas
            .copy(
                &pt1,
                None,
                Rect::new(15, 15, 128 * PIXEL_SCALE, 128 * PIXEL_SCALE),
            )
            .unwrap();
        self.canvas
            .copy(
        ...
        for pi in 0..8 {
            ...
            for i in 0..4 {
                ...
                self.canvas
                    .fill_rect(Rect::new(
                        offset_x + i * PALETTE_SCALE as i32,
                        PALETTE_Y,
                        PALETTE_SCALE,
                        PALETTE_SCALE,
                    ))
