...
mod video_interface;
pub use video_interface::{VideoInterface, DummyVideo, Color};
...
impl PPU {
    ...
    pub fn tick(&mut self, chr: &mut dyn PPUMemory, video_out: &mut dyn VideoInterface) -> () {
            ...
            0..=239 | 261 => { // Visible scanlines (And pre-render scanline)
                ...
                if (2..258).contains(&self.dot) || (321..338).contains(&self.dot) {
                    ...
                    if chr.registers().ppu_mask & 0x10 != 0 && self.dot >= 1 && self.dot < 258 {
                        for i in 0..self.sprite_count {
                            let entry = OAMEntry {
                                ...
                                tile_id: self.scanline_sprites[i as usize*4 + 1],
                                attr: self.scanline_sprites[i as usize*4 + 2],
                                x: self.scanline_sprites[i as usize*4 + 3],
                            };
                            if entry.x > 0 {
                                self.scanline_sprites[i as usize*4 + 3] -= 1;
                            } else {
                    ...
                    match (self.dot-1) % 8 {
                        ...
                        0 => { // Shift in next tile to be drawn and load NT byte for next tile
                            self.pattern_shift_low =  (self.pattern_shift_low  & 0xff00) | self.next_bg_lsb as u16;
                            self.pattern_shift_high = (self.pattern_shift_high & 0xff00) | self.next_bg_msb as u16;
                            self.palette_shift_low =  (self.palette_shift_low  & 0xff00) | (if self.next_bg_palette & 0b01 != 0 {0xff} else {0x00});
                            self.palette_shift_high = (self.palette_shift_high & 0xff00) | (if self.next_bg_palette & 0b10 != 0 {0xff} else {0x00});

                            let addr = 0x2000 + (self.vram_addr & 0b000_11_11111_11111);
                            self.next_bg_tile_id = self.read_vram(addr, chr);
                        }, 
                        2 => { // Load AT byte for next tile
                            ...
                            if self.vram_addr & 0b000_00_00010_00000 != 0 {self.next_bg_palette >>= 4}
                            if self.vram_addr & 0b000_00_00000_00010 != 0 {self.next_bg_palette >>= 2}
                            self.next_bg_palette &= 0x03;
                        },
                        4 => { // Load low BG Tile byte
                            let addr: u16 = if chr.registers().ppu_ctrl & 0x10 != 0 {0x1000} else {0x0000}
                                        + ((self.next_bg_tile_id as u16) << 4)
                                        + ((self.vram_addr & 0b111_00_00000_00000) >> 12)
                                        + 0; // First bitplane
                            self.next_bg_lsb = self.read_vram(addr, chr);
                        }, 
                        6 => { // Load high BG Tile byte
                            let addr: u16 = if chr.registers().ppu_ctrl & 0x10 != 0 {0x1000} else {0x0000}
                                        + ((self.next_bg_tile_id as u16) << 4)
                                        + ((self.vram_addr & 0b111_00_00000_00000) >> 12)
                ...
                if self.scanline == 261 && (280..305).contains(&self.dot) && chr.registers().ppu_mask & 0x18 != 0 {
                ...
                if self.dot == 340 {
                    for i in 0..self.sprite_count {
                        ...
                        let pattern_addr_lo: u16 = if chr.registers().ppu_ctrl & 0x20 == 0 {
                            ...
                            let table_addr = if chr.registers().ppu_ctrl & 0x08 != 0 { 0x1000 } else { 0 };

                            table_addr | 
                            ((entry.tile_id as u16) << 4) |
                            row_sel
                        } else {
                            // 8x16 sprite mode
                            let row_sel = if entry.attr & 0x80 == 0 { 
                            ...
                            let tile_id = if self.scanline - (entry.y as u16) < 8 { top_tile_id } else { bottom_tile_id };
        ...
        if chr.registers().ppu_mask & 0x08 != 0 && (self.dot > 8 || chr.registers().ppu_mask & 0x02 != 0) {
            let bit_mux = 0x8000 >> self.fine_x;

            let p0 = if self.pattern_shift_low & bit_mux != 0 {1} else {0};
            let p1 = if self.pattern_shift_high & bit_mux != 0 {1} else {0};
            bg_pixel = (p1 << 1) | p0;

            let pl0 = if self.palette_shift_low & bit_mux != 0 {1} else {0};
            let pl1 = if self.palette_shift_high & bit_mux != 0 {1} else {0};
            ...
            match index {
                ...
                5 if is_write => {
                    if !self.second_write {
                        // First write
                        let coarse_x = (chr.registers().ppu_scroll & 0b1111_1000) as u16 >> 3;
                        let fine_x = chr.registers().ppu_scroll & 0b0000_0111;
                        self.t_addr &= 0b111_11_11111_00000;
                        self.t_addr |= coarse_x;
                        self.fine_x = fine_x;
                        self.second_write = true;
                    } else {
                        // Second write
                        let coarse_y = (chr.registers().ppu_scroll & 0b1111_1000) as u16 >> 3;
                        let fine_y = (chr.registers().ppu_scroll & 0b0000_0111) as u16;
                        self.t_addr &= 0b000_11_00000_11111;
                        self.t_addr |= coarse_y << 5;
                        self.t_addr |= fine_y << 12;
                        self.second_write = false;
                    }
                },
                6 if is_write => {
    ...
    fn increment_scroll_x(&mut self, registers: &PPURegisters) {
        if registers.ppu_mask & 0x18 != 0 { // Check if rendering is enabled
